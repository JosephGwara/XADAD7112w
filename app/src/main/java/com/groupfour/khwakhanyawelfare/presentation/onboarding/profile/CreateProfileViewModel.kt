...
class CreateProfileViewModel @Inject constructor(private val firebaseAuth: FirebaseAuth, private val firestore: FirebaseFirestore) : ViewModel() {
    ...
    fun onOnboardingComplete(name:String,phoneNumber:String,userType: UserType) = viewModelScope.launch(Dispatchers.IO) {
        val userEmail = firebaseAuth.currentUser?.email
        if (userEmail != null){
            val user = User(name =name,number = phoneNumber,email = userEmail,userType=userType, onboardingComplete = true)
